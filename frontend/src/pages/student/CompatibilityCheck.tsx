...
import { ShieldAlert, MonitorCheck, Loader2, Check, Video, Globe, Award } from 'lucide-react';
import toast from 'react-hot-toast';

export const CompatibilityCheck: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const [browserPassed, setBrowserPassed] = useState<boolean | null>(null);
  const [networkPassed, setNetworkPassed] = useState<boolean | null>(null);
      ...
      try {
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
          ...
          await navigator.mediaDevices.getUserMedia({ video: true })
            ...
            .catch(() => {
              loggerWarning('Camera permission denied. Proceeding with simulated camera...');
  ...
  const loggerWarning = (msg: string) => {
  ...
  return (
    <div className="max-w-xl mx-auto space-y-8 animate-fade-in">
      <div className="flex flex-col items-center text-center gap-2">
        ...
        <p className="text-sm text-slate-400">Verifying browser layouts, connection quality, and proctor modules.</p>
      </div>
        ...
        <div className="flex items-center justify-between p-4 border-b border-border last:border-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Chrome / Safari / Firefox compatibility</p>
            </div>
          </div>
          <div>
            {browserPassed === null ? (
              ...
              <div className="w-6 h-6 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 rounded-full flex items-center justify-center"><Check size={14} /></div>
            ) : (
        ...
        <div className="flex items-center justify-between p-4 border-b border-border last:border-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Latency bounds & bandwidth quality</p>
            </div>
          </div>
          <div>
            {networkPassed === null ? (
        ...
        <div className="flex items-center justify-between p-4 border-b border-border last:border-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Webcam authorization validation</p>
            </div>
          </div>
          <div>
            {cameraPassed === null ? (
