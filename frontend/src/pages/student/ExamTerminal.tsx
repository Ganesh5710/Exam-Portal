...
interface Question {
  id: string;
  type: string; // MCQ, TRUE_FALSE, FILL_BLANK, DESCRIPTIVE, CODING
  content: string;
  options: string[] | null;
  score: number;
}
...
export const ExamTerminal: React.FC = () => {
        ...
        if (user) {
          ...
          if (cached) {
            try {
              ...
              toast.success('Restored previous exam progress from local cache.');
            } catch (cacheErr) {
  ...
  const handleTimeUp = () => {
    if (!isSubmitting.current) {
      toast.error('Time expired! Submitting responses automatically...', { id: 'time-up-toast' });
      triggerSubmit();
    }
  };
    ...
    const initProctor = async () => {
      try {
        ...
        try {
          ...
          toast.error('Webcam access failed. Running simulation proctor feed...', { id: 'webcam-err' });
          runSimulation();
          return;
        }
          ...
          intervalId = setInterval(async () => {
            if (videoRef.current && canvasRef.current && model) {
              ...
              if (ctx && video.readyState === 4) {
                ...
                let violationType: any = null;
                let violationDetails = '';

                if (predictions.length === 0) {
                  warning = 'NO FACE DETECTED';
                  violationType = 'NO_FACE_DETECTED';
                  violationDetails = 'Student walked away or covered the camera.';
                ...
                if (violationType) {
                  const currentSecs = violationSustainedSeconds.current[violationType] || 0;
        ...
        console.error('Proctor initialization error, using fallback simulation:', err);
        runSimulation();
      }
    };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      socket.emit('report-progress', {
        ...
        faceStatus: proctorWarning === 'NO FACE DETECTED' ? 'no_face'
          : proctorWarning === 'MULTIPLE FACES DETECTED' ? 'multiple_faces'
          : proctorWarning === 'LOOKING AWAY DETECTED' ? 'look_away'
          : 'normal'
      });
    }, 5000);
  ...
  useEffect(() => {
    ...
    const handleAnnouncement = (announcement: { message: string }) => {
    ...
    const handleTimeExtended = (data: { extensionMinutes: number }) => {
      const addedSecs = data.extensionMinutes * 60;
      setSecondsLeft(prev => prev + addedSecs);
      toast.success(`Proctor extended your exam time by ${data.extensionMinutes} minutes!`, {
        icon: '⏳',
        duration: 8000
      });
    };

    const handleForceTerminate = (data: { reason: string }) => {
      toast.error(`Exam Terminated by Proctor: ${data.reason}. Submitting answers...`, {
        icon: '⚠️',
        duration: 10000
      });
      triggerSubmit();
  ...
  useEffect(() => {
    if (exam && user && Object.keys(answers).length > 0) {
      ...
      localStorage.setItem(cacheKey, JSON.stringify({ answers, markedForReview }));
    }
  ...
  const handleTextChange = (qId: string, value: string) => {
    ...
    setAnswers(prev => ({
      ...
      [qId]: {
        value,
        ...(currentQ?.type === 'CODING' ? { language: prev[qId]?.language || selectedLang } : {})
      }
    }));
  };
      ...
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 text-center">
        <div className="max-w-md space-y-5 bg-slate-900/50 border border-slate-850 p-8 rounded-2xl">
          ...
          <p className="text-slate-400 text-sm">
            This exam does not have any questions linked to it yet. Please contact your administrator to configure questions.
          </p>
          <button
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col no-select">
      ...
      <header className="h-16 border-b border-slate-800 bg-slate-900/50 backdrop-blur px-6 flex items-center justify-between sticky top-0 z-30">
        <div>
          ...
          <p className="text-xs text-slate-400">Duration Limits: {exam?.duration} mins</p>
        </div>
        ...
        <div className="flex items-center gap-3 px-4 py-2 bg-slate-900 border border-slate-800 rounded-lg shadow-inner">
          ...
          <span className="font-mono text-lg font-bold text-violet-400">{formatTime()}</span>
        </div>
      </header>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden max-w-full">
        ...
        <div className="flex-1 p-6 md:p-8 space-y-8 overflow-y-auto max-h-[calc(100vh-8rem)]">
          <div className="flex justify-between items-center pb-4 border-b border-slate-900">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Question {currentIndex + 1} of {totalQuestions}</span>
            <span className="text-xs font-bold text-violet-400 bg-violet-500/10 px-3 py-1.5 rounded-full border border-violet-500/20">+{currentQuestion?.score} Points</span>
          </div>
            ...
            {currentQuestion?.type === 'CODING' && (
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-3 rounded-lg">
                  <span className="text-xs text-slate-400 font-semibold uppercase">Coding Workspace</span>
                  <select
                    value={selectedLang}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-slate-900">
                  <div>
                    <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Custom Input Stream (stdin)</label>
                    <textarea
                      value={customInput}
                  ...
                  <div className="flex flex-col justify-end">
                    ...
                    >
                      {executingCode ? 'Compiling & Executing...' : 'Run Code against Sandbox'}
                    </button>
                  </div>
                </div>
                ...
                {codeOutput && (
                  <div className="p-4 bg-slate-900 border border-slate-800 rounded-lg space-y-3 font-mono text-xs">
                    <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                      <span className="font-bold uppercase tracking-wider text-slate-400">Sandbox Execution Report</span>
                    ...
                    {codeOutput.status === 'COMPILATION_ERROR' ? (
                      <pre className="text-red-400 overflow-x-auto whitespace-pre-wrap">{codeOutput.logs}</pre>
                    ) : (
                      <div className="space-y-2">
                        {codeOutput.results.map((res: any, idx: number) => (
                          <div key={idx} className="space-y-1">
                            <p className="font-semibold text-slate-400">Test Case {idx + 1} ({res.timeTakenMs}ms): {res.passed ? <span className="text-emerald-400">PASSED</span> : <span className="text-red-400">FAILED</span>}</p>
                            <div className="bg-slate-950 p-2.5 border border-slate-800 rounded space-y-1 mt-1 text-[11px]">
                              <p><span className="text-slate-500 font-bold">Input:</span> {res.input}</p>
                              <p><span className="text-slate-500 font-bold">Expected:</span> {res.expected}</p>
                              <p><span className="text-slate-500 font-bold">Output:</span> {res.actual || res.error || '(empty)'}</p>
                            </div>
                          </div>
                        ))}
                      </div>
          ...
          <div className="flex justify-between items-center pt-8 border-t border-slate-900">
            <div className="flex gap-2">
              ...
              <button
                onClick={() => setMarkedForReview(p => ({ ...p, [currentQuestion.id]: !p[currentQuestion.id] }))}
        ...
        <aside className="w-full lg:w-80 border-t lg:border-t-0 lg:border-l border-slate-800 bg-slate-900/30 p-6 space-y-6 max-h-[calc(100vh-4rem)] overflow-y-auto">
          ...
          <div className="glass-card p-4 rounded-xl border border-slate-800 space-y-3 relative overflow-hidden bg-slate-900/40">
            <div className="flex items-center justify-between">
              ...
              <span className={`w-2 h-2 rounded-full ${proctorActive ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`} />
            </div>
            ...
            <div className="text-[11px] text-slate-500 space-y-1 pt-1 border-t border-slate-800/50 flex justify-between">
              <span className="flex items-center gap-1">
                <UserCheck size={12} className={proctorWarning ? 'text-red-400' : 'text-emerald-400'} />
                Face Count: <strong className="text-slate-300">{faceCount}</strong>
              </span>
              <span>
                Status: <strong className={proctorWarning ? 'text-red-400' : 'text-emerald-400'}>{proctorWarning ? 'FLAGGED' : 'NORMAL'}</strong>
              </span>
            </div>
          </div>

          <div>
            <h3 className="font-bold text-sm uppercase tracking-wider text-slate-400 mb-4">Question Palette</h3>
                ...
                return (
                  <button
                    ...
                    className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm border transition-all duration-200
                      ${isCurrent ? 'bg-primary border-primary text-white ring-2 ring-primary/40'
                        : isMarked ? 'bg-amber-500/10 border-amber-500/30 text-amber-400'
          ...
          <div className="border-t border-slate-800 pt-6 space-y-3">
            <h4 className="font-bold text-xs uppercase tracking-wider text-slate-500">Legend</h4>
