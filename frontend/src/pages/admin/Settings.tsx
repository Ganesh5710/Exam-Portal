...
export const Settings: React.FC = () => {
  ...
  const handleRestoreBackup = async (fileName: string) => {
    const confirm = window.confirm(`Warning: Restoring backup "${fileName}" will overwrite current data. Proceed?`);
    if (!confirm) return;
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold tracking-tight">System Settings & Backups</h1>
        <p className="text-muted-foreground mt-1">Configure global portal parameters and manage database rollbacks.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Settings form Column */}
        <div className="lg:col-span-2 space-y-6">
          <form onSubmit={handleSaveSettings} className="glass-card p-6 rounded-xl border border-border space-y-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Institution Name</label>
                <input
                  type="text"
                  value={settings.INSTITUTION_NAME}
                  onChange={(e) => handleUpdateSetting('INSTITUTION_NAME', e.target.value)}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Portal Theme</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Session Timeout (m)</label>
                  <input
                    type="number"
            ...
            <div className="space-y-4 pt-4 border-t border-border">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="col-span-2">
                  <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">SMTP Host</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">SMTP Port</label>
                  <input
                    type="text"
            ...
            <div className="space-y-4 pt-4 border-t border-border">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Blocks all student access immediately.</p>
                </div>
                <button
                  type="button"
            ...
            >
              <Save size={16} /> {submitting ? 'Saving settings...' : 'Commit Configurations'}
            </button>
          </form>
        </div>
          ...
          <div className="glass-card p-6 rounded-xl border border-border space-y-6 flex flex-col justify-between min-h-[400px]">
            <div>
              ...
              <div className="mt-6 space-y-4 overflow-y-auto max-h-[300px]">
                {backups.length === 0 ? (
                  <p className="text-xs text-muted-foreground text-center py-6">No historical backups found.</p>
                ) : (
                  backups.map((backup) => (
                    <div key={backup.fileName} className="flex justify-between items-center p-3 bg-accent/30 border border-border rounded-lg text-xs">
                      <div>
                        <p className="font-semibold truncate max-w-[150px]">{backup.fileName}</p>
                        <span className="text-[10px] text-muted-foreground">Size: {(backup.size / 1024).toFixed(1)} KB</span>
                      </div>
