...
interface Exam {
  ...
  _count?: { examQuestions: number; assignments: number };
  createdAt: string;
}
...
export const ExamsPortal: React.FC = () => {
  ...
  const [statusTab, setStatusTab] = useState<'ALL' | 'DRAFT' | 'PUBLISHED' | 'COMPLETED'>('ALL');
  ...
  const fetchQuestionsForSubject = async (subjId: string) => {
    if (!subjId) return;
    try {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.title || !formData.startTime || !formData.endTime || !formData.subjectId) {
  ...
  const openEditModal = (exam: Exam) => {
    ...
    const parseLocalTime = (isoStr: string) => {
    ...
    api.get(`/exams/${exam.id}/questions`).then(res => {
      const ids = res.data.data?.questions?.map((q: any) => q.id) || [];
  ...
  const handleToggleQuestion = (qId: string) => {
    ...
    if (current.includes(qId)) {
      setFormData({ ...formData, selectedQuestions: current.filter(id => id !== qId) });
    } else {
  ...
  const someSelected = filteredExams.some(e => selectedIds.has(e.id)) && !allSelected;
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await api.delete('/exams/bulk', { data: { ids: Array.from(selectedIds) } });
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'COMPLETED': return 'bg-violet-500/10 border-violet-500/20 text-violet-400';
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white">Exams Portal</h1>
          <p className="text-slate-400 mt-1 text-sm">Schedule assessments, configure policies, and assign candidate lists.</p>
        </div>
        <button
          onClick={() => {
            resetForm();
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
            ...
            {(['ALL', 'DRAFT', 'PUBLISHED', 'COMPLETED'] as const).map(tab => (
              <button
                ...
                className={`px-3 py-1.5 rounded-md transition-all ${
                  statusTab === tab ? 'bg-violet-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
          ...
          >
            {allSelected ? <CheckSquare size={14} className="text-violet-400" /> : someSelected ? <CheckSquare size={14} className="text-violet-400/50" /> : <Square size={14} />}
            Select All
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-slate-900 border border-slate-800/40 rounded-xl">
          ...
          <h3 className="text-lg font-semibold text-slate-300">No Exams Scheduled</h3>
          <p className="text-sm text-slate-500">Add an exam to begin evaluating student knowledge.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredExams.map((exam) => {
            ...
            return (
              <div key={exam.id} className={`relative bg-slate-900/80 border rounded-xl p-5 hover:bg-slate-900 transition-all flex flex-col justify-between space-y-4 ${
                isSelected ? 'border-violet-500 bg-violet-950/10 shadow-lg shadow-violet-500/5' : 'border-slate-800'
              }`}>
                <div className="space-y-2">
                  <div className="flex justify-between items-start gap-2">
                    <div className="flex items-center gap-2">
                      <button onClick={() => toggleSelect(exam.id)} className="text-slate-500 hover:text-violet-400 transition-colors shrink-0">
                        {isSelected ? <CheckSquare size={16} className="text-violet-400" /> : <Square size={16} />}
                      </button>
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${getStatusColor(exam.status)}`}>
                        {exam.status}
                      </span>
                    </div>
                    <span className="text-xs text-slate-500">{exam.subject?.code || 'GEN'}</span>
                  </div>
                  <h3 className="font-bold text-white text-base leading-snug truncate">{exam.title}</h3>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs border-y border-slate-850 py-3 text-slate-400">
                <div className="flex items-center gap-1.5"><Clock size={13} /> {exam.duration} mins</div>
                <div className="flex items-center gap-1.5"><FileText size={13} /> {exam._count?.examQuestions || 0} Questions</div>
                <div className="flex items-center gap-1.5"><UserCheck size={13} /> {exam._count?.assignments || 0} Assigned</div>
                <div className="flex items-center gap-1.5"><Calendar size={13} /> {new Date(exam.startTime).toLocaleDateString()}</div>
              </div>
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-4 px-6 py-3.5 bg-slate-900 border border-violet-500/30 rounded-2xl shadow-2xl shadow-violet-500/10 backdrop-blur-md">
          <div className="flex items-center gap-2 text-sm font-semibold text-white">
            ...
            <span className="text-violet-400">{selectedIds.size}</span> exam{selectedIds.size !== 1 ? 's' : ''} selected
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-slate-900 border border-red-500/30 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-800 bg-red-500/5">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">Delete {selectedIds.size} Exams?</h2>
                <p className="text-xs text-slate-400">This permanently deletes all selected exams, assignments, questions mappings, and student submissions.</p>
              </div>
            </div>
            <div className="px-6 py-5">
              <p className="text-sm text-slate-400">This action <span className="text-red-400 font-semibold">cannot be undone</span>. All associated settings, question assignments, user mappings, and submission records for the selected exams will be permanently deleted.</p>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-3xl mx-4 bg-slate-900 border border-slate-800 rounded-2xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 space-y-5">
            <div className="flex justify-between items-center pb-3 border-b border-slate-800">
              <h3 className="text-lg font-bold text-white">Create Examination Schedule</h3>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase mb-2">Subject Course</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase mb-2">Duration (Minutes)</label>
                  <input
                    type="number"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase mb-2">Scheduled Start Time</label>
                  <input
                    type="datetime-local"
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase mb-2">Scheduled End Time</label>
                  <input
                    type="datetime-local"
                    ...
                    {questions.length === 0 ? (
                      ...
                      questions.map(q => (
                        <label key={q.id} className="flex items-center justify-between text-xs text-slate-300 border-b border-slate-900 pb-1.5 cursor-pointer">
                          ...
                          <span className="font-semibold text-violet-400">+{q.score} pts</span>
                        </label>
                      ))
                    )}
                  </div>
      ...
      {showAssignModal && selectedExam && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-xl mx-4 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 space-y-5">
            <div className="flex justify-between items-center pb-3 border-b border-slate-800">
              <div>
                <h3 className="text-lg font-bold text-white">Assign Exam to Students</h3>
                <p className="text-xs text-slate-500 mt-0.5">{selectedExam.title}</p>
              </div>
              <button onClick={() => setShowAssignModal(false)} className="text-slate-400 hover:text-white"><X size={18} /></button>
            </div>
            ...
            <form onSubmit={handleAssignStudents} className="space-y-4">
              <div className="border border-slate-850 bg-slate-950 rounded-lg p-3 max-h-[300px] overflow-y-auto space-y-3">
                {students.length === 0 ? (
                  <p className="text-xs text-slate-600 text-center py-4">No student records found.</p>
                ) : (
                  students.map(s => (
                    <label key={s.id} className="flex items-center gap-3 text-xs text-slate-300 border-b border-slate-900 pb-2 cursor-pointer last:border-b-0 last:pb-0">
                      <input
                        type="checkbox"
      ...
      {showDeleteModal && selectedExam && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md mx-4 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 text-center space-y-4">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Delete Assessment</h2>
              <p className="text-sm text-slate-400 mt-2">
                Are you sure you want to permanently delete the exam{' '}
                <span className="font-semibold text-white">"{selectedExam.title}"</span>? This action cannot be undone and will delete student results!
              </p>
            </div>
