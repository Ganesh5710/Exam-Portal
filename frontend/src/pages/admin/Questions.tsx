...
interface Question {
  ...
  content: string;
  options: any; // string[]
  answers: any; // string[] or { testCases: [] }
...
export const Questions: React.FC = () => {
  ...
  const handleGenerateAI = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('AI successfully generated candidate questions! Please review them.');
    } catch (err: any) {
      toast.error(err.response?.data?.message || 'Failed to generate questions. Verify GEMINI_API_KEY.');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const handleSaveAIQuestions = async () => {
    ...
    try {
      ...
      toast.success('Successfully imported all AI generated questions into the bank!');
      setShowAIModal(false);
      setAiPreviewQuestions([]);
      fetchQuestions();
    } catch (err: any) {
      toast.error(err.response?.data?.message || 'Failed to import AI questions.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Type', 'Content', 'Options', 'Answers', 'Explanation', 'Score', 'NegativeMarks', 'Difficulty', 'Tags', 'SubjectName', 'SubjectCode'];
    const rows = questions.map(q => {
      const optionsStr = Array.isArray(q.options) ? q.options.join('; ') : '';
      const answersStr = Array.isArray(q.answers) ? q.answers.join('; ') 
        : typeof q.answers === 'object' ? JSON.stringify(q.answers) 
        : String(q.answers);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (formData.type === 'FILL_BLANK' || formData.type === 'DESCRIPTIVE') {
        payloadOptions = null;
      ...
      const body = {
        ...
        tags: formData.tagsString ? formData.tagsString.split(',').map(t => t.trim()) : [],
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await api.delete('/questions/bulk', { data: { ids: Array.from(selectedIds) } });
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'MCQ': return 'bg-violet-500/10 text-violet-400 border border-violet-500/20';
      case 'CODING': return 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20';
      case 'TRUE_FALSE': return 'bg-blue-500/10 text-blue-400 border border-blue-500/20';
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white">Questions Bank</h1>
          <p className="text-slate-400 mt-1 text-sm">Create and organize questions across different subject modules.</p>
        </div>
        <div className="flex gap-2 relative">
          <div className="relative group">
            <button
              className="flex items-center gap-2 px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-semibold rounded-lg text-sm transition-all border border-slate-750"
            >
              <Download size={18} />
              Export
            </button>
          ...
          <button
            onClick={() => {
              setAiFormData(prev => ({ ...prev, subjectId: subjects[0]?.id || '' }));
              setAiPreviewQuestions([]);
              setShowAIModal(true);
            }}
      ...
      <div className="bg-slate-900/80 border border-slate-800/50 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-slate-500">Create new questions or adjust search filters.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-850">
                  ...
                  <th className="text-left text-xs font-semibold text-slate-400 uppercase px-6 py-4">Content</th>
                  <th className="text-left text-xs font-semibold text-slate-400 uppercase px-6 py-4">Type</th>
                  <th className="text-left text-xs font-semibold text-slate-400 uppercase px-6 py-4">Subject</th>
                  <th className="text-left text-xs font-semibold text-slate-400 uppercase px-6 py-4">Difficulty</th>
                  <th className="text-left text-xs font-semibold text-slate-400 uppercase px-6 py-4">Marks</th>
                  <th className="text-right text-xs font-semibold text-slate-400 uppercase px-6 py-4">Actions</th>
                </tr>
              </thead>
              <tbody>
                {questions.map((q) => (
                  <tr key={q.id} className={`border-b border-slate-850 hover:bg-slate-800/35 transition-colors ${selectedIds.has(q.id) ? 'bg-violet-500/5' : ''}`}>
                    <td className="px-4 py-4">
                      <button onClick={() => toggleSelect(q.id)} className="text-slate-500 hover:text-violet-400 transition-colors">
                        {selectedIds.has(q.id) ? <CheckSquare size={16} className="text-violet-400" /> : <Square size={16} />}
                      </button>
                    </td>
                    <td className="px-6 py-4 font-medium text-white max-w-md truncate">{q.content}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-0.5 rounded text-xs font-semibold ${getTypeColor(q.type)}`}>
                        {q.type}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-slate-300">{q.subject?.name || 'Unknown'}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-0.5 rounded text-xs font-semibold border ${getDifficultyColor(q.difficulty)}`}>
                        {q.difficulty}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => { setSelectedQuestion(q); setShowDeleteModal(true); }}
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-4 px-6 py-3.5 bg-slate-900 border border-violet-500/30 rounded-2xl shadow-2xl shadow-violet-500/10 backdrop-blur-md">
          <div className="flex items-center gap-2 text-sm font-semibold text-white">
            ...
            <span className="text-violet-400">{selectedIds.size}</span> question{selectedIds.size !== 1 ? 's' : ''} selected
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-slate-900 border border-red-500/30 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-800 bg-red-500/5">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">Delete {selectedIds.size} Questions?</h2>
                <p className="text-xs text-slate-400">This permanently removes all selected questions and unlinks them from any exams.</p>
              </div>
            </div>
            <div className="px-6 py-5">
              <p className="text-sm text-slate-400">This action <span className="text-red-400 font-semibold">cannot be undone</span>. All selected questions will be permanently deleted from the question bank and removed from all associated exams.</p>
            </div>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              {formData.type === 'MCQ' && (
                <div className="space-y-3 p-3 bg-slate-950 border border-slate-850 rounded-lg">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Answer Choices Options</span>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Explanation (Optional)</label>
                <textarea
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md mx-4 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 text-center space-y-4">
            ...
            <div className="bg-slate-950/50 border border-slate-800/50 rounded-lg p-3 text-left">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Question Content Preview</p>
              <p className="text-xs text-slate-300 font-mono truncate">{selectedQuestion.content}</p>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-2xl mx-4 bg-slate-900 border border-slate-800 rounded-2xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 space-y-5">
            <div className="flex justify-between items-center pb-3 border-b border-slate-800">
              ...
              <button onClick={() => setShowAIModal(false)} className="text-slate-400 hover:text-white"><X size={18} /></button>
            </div>
            ...
            <form onSubmit={handleGenerateAI} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setAiFormData({ ...aiFormData, subjectId: e.target.value })}
                    className="w-full p-2.5 bg-slate-950 border border-slate-800 rounded-lg text-sm text-white"
                    required
                  >
                    <option value="">Select Subject</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Questions Count</label>
                  <input
                    type="number"
                    min="1"
                    max="10"
                    value={aiFormData.count}
                    onChange={(e) => setAiFormData({ ...aiFormData, count: Number(e.target.value) })}
                    className="w-full p-2.5 bg-slate-950 border border-slate-800 rounded-lg text-sm text-white"
                    required
                  />
                </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">AI Topic / Syllabus Details</label>
                <input
                  type="text"
            ...
            {aiPreviewQuestions.length > 0 && (
              <div className="border-t border-slate-800 pt-4 space-y-4 max-h-[40vh] overflow-y-auto">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Generated Questions Preview</span>
                  ...
                  {aiPreviewQuestions.map((q, idx) => (
                    <div key={idx} className="p-3.5 bg-slate-950 border border-slate-850 rounded-lg space-y-2">
                      <div className="flex justify-between items-center text-[10px] text-slate-500">
                        <span className="font-semibold text-slate-400">Question {idx + 1} ({q.type})</span>
                        <span className="text-emerald-400">+{q.score} Points</span>
                      </div>
                      <p className="text-xs text-slate-200 leading-relaxed font-semibold">{q.content}</p>
                      {q.options && q.options.length > 0 && (
                        <div className="grid grid-cols-2 gap-2 pl-2">
                          {q.options.map((opt: string, i: number) => (
                            <span key={i} className="text-[10px] text-slate-500 font-mono">{String.fromCharCode(65+i)}. {opt}</span>
                          ))}
                        </div>
                      )}
                      <p className="text-[10px] text-slate-400"><strong className="text-slate-500">Answer:</strong> {Array.isArray(q.answers) ? q.answers.join(', ') : typeof q.answers === 'object' ? 'Predefined Test Cases' : q.answers}</p>
                    </div>
                  ))}
                </div>
