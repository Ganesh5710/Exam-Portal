...
export const Dashboard: React.FC = () => {
  ...
  const fetchDashboardData = async (isRefresh = false) => {
    ...
    try {
      const [usersRes, deptsRes, subjectsRes, questionsRes, examsRes, submissionsRes] =
      ...
      if (usersRes.status === 'fulfilled') {
        ...
        if (usersData?.students) {
          totalStudents = usersData.pagination?.total || usersData.students.length;
      ...
      if (examsRes.status === 'fulfilled') {
        ...
        if (Array.isArray(examsData)) {
          activeExams = examsData.filter((e: any) => e.status === 'PUBLISHED').length;
          completedExams = examsData.filter((e: any) => e.status === 'COMPLETED').length;
        }
      }
      ...
      if (submissionsRes.status === 'fulfilled') {
        ...
        const submissions = submissionsData?.submissions || (Array.isArray(submissionsData) ? submissionsData : []);
        if (submissions.length > 0) {
          const completedSubs = submissions.filter((s: any) => s.status === 'COMPLETED' || s.totalScore !== undefined);
          if (completedSubs.length > 0) {
            const totalScore = completedSubs.reduce((sum: number, s: any) => sum + (s.totalScore || 0), 0);
            averageScore = Math.round((totalScore / completedSubs.length) * 10) / 10;
            const passed = completedSubs.filter((s: any) => (s.totalScore || 0) >= 40).length;
      ...
      if (examsRes.status === 'fulfilled') {
        const examsData = examsRes.value.data?.data;
        if (Array.isArray(examsData)) {
          const publishedExams = examsData.filter((e: any) => e.status === 'PUBLISHED');
          publishedExams.slice(0, 2).forEach((exam: any, idx: number) => {
            activities.push({
              id: `exam-pub-${idx}`,
              text: `Exam "${exam.title || 'Untitled'}" is currently live`,
              time: exam.updatedAt ? formatRelativeTime(exam.updatedAt) : 'Recently',
              type: 'success',
            });
          });
          const draftExams = examsData.filter((e: any) => e.status === 'DRAFT');
          draftExams.slice(0, 1).forEach((exam: any, idx: number) => {
            activities.push({
              id: `exam-draft-${idx}`,
      ...
      if (usersRes.status === 'fulfilled') {
        const usersData = usersRes.value.data?.data;
        const students = usersData?.students || (Array.isArray(usersData) ? usersData : []);
        const recentStudents = [...students]
          .sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
          .slice(0, 2);
        recentStudents.forEach((student: any, idx: number) => {
          activities.push({
            id: `student-${idx}`,
            text: `Student ${student.firstName || ''} ${student.lastName || ''} registered`.trim(),
            time: student.createdAt ? formatRelativeTime(student.createdAt) : 'Recently',
            type: 'info',
          });
        });
      }
      ...
      if (activities.length === 0) {
        activities.push(
          { id: 'placeholder-1', text: 'System initialized and operational', time: 'Just now', type: 'success' },
          { id: 'placeholder-2', text: 'Dashboard data synced successfully', time: 'Just now', type: 'info' },
        );
      }
        ...
        {statCards.map((card, i) => {
          ...
          return (
            ...
            >
              ...
              <div className={`absolute inset-0 ${card.bgColor} opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl`} />
        ...
        <div className="bg-slate-900/80 border border-slate-800/50 rounded-xl p-6 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2.5 mb-6">
              ...
              <h3 className="font-semibold text-lg text-white">Quick Overview</h3>
            </div>
              ...
              <div className="mt-6 pt-5 border-t border-slate-800">
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center p-3 bg-emerald-500/5 border border-emerald-500/10 rounded-lg">
                    <div className="text-2xl font-bold text-emerald-400">{stats.activeExams}</div>
                  ...
                  <div className="text-center p-3 bg-slate-800/50 border border-slate-700/50 rounded-lg">
                    <div className="text-2xl font-bold text-slate-300">{stats.completedExams}</div>
