...
export const useAntiCheat = ({
  ...
  useEffect(() => {
    ...
    const preventRightClick = (e) => {
      e.preventDefault();
      toast.error("Right-click is disabled during exams.", {
        id: "cheat-right-click",
      });
    };
      ...
      if (document.hidden) {
        setTabSwitches((prev) => {
          ...
          triggerViolationAlert(
            "TAB_SWITCH",
            `Switched away from browser tab. Count: ${newCount}`,
          );
          return newCount;
        });
      }
    ...
    const handleFullscreenChange = () => {
      if (
        ...
        setFullscreenExits((prev) => {
          ...
          triggerViolationAlert(
            "FULLSCREEN_EXIT",
            `Exited fullscreen mode. Count: ${newCount}`,
          );
          return newCount;
        });
        toast.error(
          "Fullscreen mode is required. Please re-enter fullscreen.",
          { id: "fullscreen-warn", duration: 5000 },
        );
      }
    };

    const triggerViolationAlert = (type, details) => {
      if (socket) {
  ...
  useEffect(() => {
    ...
    if (totalViolations >= maxViolations && !isSubmitting.current) {
      ...
      toast.error(
        "Maximum cheating warnings exceeded. Auto-submitting exam...",
        {
          duration: 10000,
  ...
  const enterFullscreen = () => {
    ...
    if (element.requestFullscreen) {
      element.requestFullscreen().catch(() => {
        toast.error(
          "Failed to request fullscreen. Ensure browser has permissions.",
        );
      });
    }
  };
