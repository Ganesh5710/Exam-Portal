...
import { executeCode } from './codeExecution.service';
...
export const bulkImportQuestions = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    for (const record of questions) {
      ...
      if (!sub) continue; // Skip if subject doesn't exist
...
export const runQuestionCode = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const report = await executeCode(language, code, testCases);
...
const LOCAL_QUESTIONS_DB: Record<string, any[]> = {
  'binary search': [
    {
      type: 'MCQ',
...
function generateQuestionsLocally(topic: string, type: string, count: number): any[] {
...
export const generateAIQuestions = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
...
Return ONLY a JSON array of question objects. Do not wrap it in markdown code blocks or add any conversational text. Just return the raw JSON array.`;
    ...
    const result = await callGeminiWithFallback(geminiApiKey, { prompt });

    const generatedQuestions = JSON.parse(result.text);

    // Enrich questions with subjectId
    const enriched = (Array.isArray(generatedQuestions) ? generatedQuestions : [generatedQuestions]).map((q: any) => ({
      ...q,
      subjectId
    }));
    ...
    const enriched = localQuestions.map((q: any) => ({
      ...q,
      subjectId
    }));
...
export const bulkDeleteQuestions = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  const { ids } = req.body;
  if (!Array.isArray(ids) || ids.length === 0)
    return res.status(400).json({ success: false, message: 'Provide an array of question IDs.' });
  try {
    await prisma.examQuestion.deleteMany({ where: { questionId: { in: ids } } });
