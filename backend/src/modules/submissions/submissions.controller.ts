...
import { executeCode } from '../questions/codeExecution.service';

export const saveAnswers = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
...
export const submitExam = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const getCorrectArray = (rawAnswers: any): string[] => {
    ...
    for (const question of examQuestions) {
      ...
      const studentResponse = savedAns?.studentAnswer as any;
      ...
      if (!studentResponse) {
        ...
        const selectedOpts: string[] = Array.isArray(studentResponse.selectedOptions)
        ...
        const answersObj = question.answers as any;
        const testCases: any[] = Array.isArray(answersObj?.testCases) ? answersObj.testCases : [];

        if (studentCode.trim() && testCases.length > 0) {
          try {
            const report = await executeCode(studentLanguage as any, studentCode, testCases);
            const totalCases = testCases.length;
            const passedCases = report.results.filter((r: any) => r.passed).length;
...
export const getSubmissionsForExam = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
...
export const gradeDescriptiveAnswer = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const submissionAnswers = await prisma.answer.findMany({
    ...
    const exam = await prisma.exam.findUnique({ where: { id: answer.submission.examId } });
...
export const updateSubmission = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  const { id } = req.params;
  ...
  try {
    ...
    if (totalScore !== undefined) {
      ...
      else if (percentage >= 40) gradeVal = 'D';
      ...
      if (isPassed !== undefined) updateData.isPassed = isPassed;
    }

    const updated = await prisma.submission.update({ where: { id }, data: updateData });
...
export const deleteSubmission = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  const { id } = req.params;
...
export const bulkDeleteSubmissions = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  const { ids } = req.body;
